...
Mat fromNDArrayToMat(PyObject* o) {
	std::cout << "cv::Mat m" << std::endl;
	cv::Mat m;
	bool allowND = true;
	std::cout << "!PyArray_Check(o)" << std::endl;
	if (0) {
		std::cout << "failmsg(argument is not a numpy array);" << std::endl;
		failmsg("argument is not a numpy array");
		if (!m.data)
			m.allocator = &g_numpyAllocator;
	} else {
		std::cout << "m.allocator = &g_numpyAllocator" << std::endl;
		PyArrayObject* oarr = (PyArrayObject*) o;
		std::cout << "PyArrayObject* oarr = (PyArrayObject*) o;" << std::endl;
