...
void encryptString(const string& in, byte key[16], byte* out) {
    ...
    //truncate the 128 times of the string
    int end = len % 16;
    string part = in.substr(0, len - end), tail = in.substr(len - end);
    assert(part.size() % 16 == 0);
...
string crackString(byte* in, byte key[16], int len) {
    ...
    if (len < 16){
        for (int k = 0; k < len; k++) {
            result += static_cast<char>(in[k].to_ulong());
        }
        return result;
    }
    //truncate the 128 times of the string
    int end = len % 16, N = len / 16;
    ...
    for (int s = 0; s < N; s++) {
        ...
        decrypt(seg, w);
        for (i = 0; i < 16; i++) {
            result += static_cast<char>(seg[i].to_ulong());
        }
    }
    //unencryption tails
    for(int l = len - end; l < len; l++)
        result  += static_cast<char>(in[l].to_ulong());
    return result;
}
